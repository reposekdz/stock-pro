...
const InteractiveLogo = ({ onClick }: { onClick: () => void }) => {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-emerald-500 rounded-full transition-all duration-500 ease-out group-hover:scale-110 group-hover:rotate-180 group-hover:bg-gradient-to-br group-hover:from-emerald-400 group-hover:to-teal-600 shadow-lg shadow-emerald-200/50"></div>
...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className={`fixed top-4 left-4 right-4 z-50 transition-all duration-500 ease-in-out ${zenMode ? '-translate-y-32 opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
      <div className="bg-white/80 backdrop-blur-xl rounded-full px-3 py-2.5 shadow-2xl border border-white/40 flex items-center justify-between gap-3 md:gap-4 relative overflow-hidden">
        
        {/* Shine Effect */}
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] animate-[shine_8s_infinite]"></div>
          ...
          <div className="hidden md:flex items-center gap-2">
              <button 
                  ...
                  className={`relative p-2.5 rounded-full transition-all duration-500 overflow-hidden group
                      ${canGoBack 
                          ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg hover:shadow-emerald-200 hover:scale-110 active:scale-95' 
                          : 'bg-gray-100 text-gray-300 cursor-default opacity-50'}`}
              >
                  <ArrowLeft size={20} strokeWidth={3} className={canGoBack ? "group-hover:-translate-x-1 transition-transform" : ""} />
              </button>
              ...
              <button 
                  ...
                  className={`relative p-2.5 rounded-full transition-all duration-500 overflow-hidden group
                      ${canGoForward 
                          ...
                          : 'bg-gray-100 text-gray-300 cursor-default opacity-50'}`}
              >
                  <ArrowRight size={20} strokeWidth={3} className={canGoForward ? "group-hover:translate-x-1 transition-transform" : ""} />
              </button>
          </div>
        </div>
        ...
        <div className="flex-1 relative group max-w-2xl mx-auto" ref={searchRef}>
          ...
          <input 
            ...
            className="w-full bg-gray-100/80 hover:bg-white focus:bg-white focus:ring-2 focus:ring-emerald-400/50 text-gray-900 rounded-full py-3 pl-12 pr-24 outline-none transition-all duration-300 shadow-inner focus:shadow-xl font-medium placeholder:text-gray-400 text-base"
            value={searchValue}
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
              ...
              <button 
                  className="p-2 hover:bg-emerald-50 rounded-full text-gray-500 hover:text-emerald-600 hover:shadow-md transition-all active:scale-90 group/cam" 
                  title="Visual Search"
          ...
          {showSuggestions && (
              <div className="absolute top-full left-0 right-0 mt-4 bg-white/80 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/50 overflow-hidden py-4 animate-in fade-in slide-in-from-top-4 duration-300 z-50">
        ...
        <div className="flex items-center gap-1 md:gap-2 text-gray-500">
          ...
          <button className="p-3 hover:bg-gray-100 rounded-full relative hidden sm:block transition-transform hover:scale-110 active:scale-95 hover:text-emerald-600">
            <Bell size={24} />
            <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white animate-pulse"></span>
          </button>
          
          <button className="p-3 hover:bg-gray-100 rounded-full hidden sm:block transition-transform hover:scale-110 active:scale-95 hover:text-emerald-600">
          ...
          >
            <div className="w-11 h-11 rounded-full bg-gradient-to-tr from-emerald-400 to-teal-500 p-[2px] shadow-lg shadow-emerald-200">
      ...
      {zenMode && (
         <button 
            className="fixed top-6 right-6 z-[60] bg-black/80 text-white px-4 py-2 rounded-full backdrop-blur-md shadow-2xl hover:bg-black transition cursor-pointer pointer-events-auto"
            onClick={toggleZenMode}
         >
             Exit Zen Mode
         </button>
